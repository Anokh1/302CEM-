            ...
            <?php foreach ($product_shuffle as $item) { ?>
                <div class="item py-2">
                    <div class="product font-rale">
                        <a href="<?php printf('%s?barcodeNumber=%s', 'product.php', $item['barcodeNumber']); ?>">
                        ...
                        <div class="text-center">
                            ...
                            <form method="post">
                                <input
                                        type="hidden"
                                        name="barcodeNumber"
                                        value="<?php echo $item['barcodeNumber'] ?? '1'; ?>"
                                >
                                <input
                                        type="hidden"
                                        name="user_id"
                                        value="<?php echo 1; ?>"
                                > <!-- user id is the first user cause there is no USER SESSION -->

                                <!-- change the button to prevent user from adding it to cart again -->
                                <?php
                                if (in_array($item['barcodeNumber'], $Cart->getCartId($product->getData('cart')) ?? [])) {
                                    echo '<button type="submit" 
                                       disabled class="btn btn-success 
                                       font-size-12">In the Cart
                                       </button>';
                                } else {
                                    echo '<button type="submit" 
                                       name="top_sale_submit" 
                                       class="btn btn-warning 
                                       font-size-12">Add to Cart
                                       </button>';
                                }
                                ?>
